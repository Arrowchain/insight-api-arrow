...
StatisticsController.prototype.minedSupply = function(req, res) {
  var cb = function (err, response) {
    if (err) {
      return res.status(500).send('error getting mined supply')
    }
    return res.status(200).send(response.mined_currency_amount)
  }
  return this.statisticService.getTotal(cb)
}
